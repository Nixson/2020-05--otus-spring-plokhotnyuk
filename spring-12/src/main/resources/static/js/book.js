...
function Book() {
    ...
    self.addComment = () => {
        self.addType = 'comment';
        $('#bookAddSubElement .modal-title').text('Add comment');
        $('#bookAddSubElement #bookAddNameLabel').text('Content');
        $('#bookAddSubElement form')[0].reset();
        $('#bookAddSubElement').modal('show');
    };
    self.submit = () => {
    ...
    self.addSubelement = () => {
        ...
        let data = {name: $('#bookAddName').val()};
        if (self.addType === 'comment') {
            data = {content: $('#bookAddName').val()};
        }
        util.post('/' + self.addType + '/' + self.active, data)
    ...
    self.remove = () => {
        if (confirm("Are you sure?")) {
            util.delete(url + "/" + self.active).then(() => self.reload());
        }
    };
    self.tplView = `
        ...
        <div class="collapse" id="collapsedComments{{id}}">
            <div class="d-flex justify-content-end mb-3">
                <button type="button" class="btn btn-success btn-sm action d-none" data-action="addComment">
                <span aria-hidden="true">&#43;</span></button>
            </div>
